...
function GameMode({ game, showStats, setShowStats }) {
  ...
  return (
    <div className="btn-group relative top-0.5">
      <button className={`btn btn-sm ${!practice && 'btn-active'}`} onClick={practiceOff}>Daily {wonDaily && '✅'}</button>
      <button className={`btn btn-sm ${practice && 'btn-active'}`} onClick={practiceButton}>Practice</button>
